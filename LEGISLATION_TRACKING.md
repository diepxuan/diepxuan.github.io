# LEGISLATION_TRACKING.md

> **Đệ #1 Discovery — Báo cáo văn bản pháp luật mới**
> Phiên bản: 2026-08-18T19:15 GMT+7
> Nguồn quét: luatvietnam.vn (Sitemap)
> **Ghi chú lần quét này**: Phát hiện thêm 5 văn bản mới (Nghị định 322/2026/NĐ-CP, 319/2026/NĐ-CP, 323/2026/NĐ-CP, Thông tư 18/2026/TT-BNV, 04/2026/TT-BDTTG).

---

## TÓM TẮT

- Tổng văn bản phát hiện lần này: **5**
- Văn bản đã có trong tracking/workspace: **0**
- Nguồn xác nhận: luatvietnam.vn
- Ngày quét: 2026-08-18

---

## VĂN BẢN MỚI PHÁT HIỆN (5) — 2026-08-18

### 1. Nghị định 322/2026/NĐ-CP — Sửa đổi Nghị định 86/2021 về học tập và nghiên cứu nước ngoài
- **Slug**: 444451
- **Trạng thái**: Chưa có

### 2. Nghị định 319/2026/NĐ-CP — Quy định thiết bị giám sát hành trình và ghi nhận hình ảnh trên phương tiện giao thông
- **Slug**: 444445
- **Trạng thái**: Hoàn thiện

### 3. Nghị định 323/2026/NĐ-CP — Quy định chi tiết và hướng dẫn thi hành Nghị quyết 20/2026/QH16
- **Slug**: 444444
- **Trạng thái**: Chưa có

### 4. Thông tư 18/2026/TT-BNV — Quy định tặng kỷ niệm chương ngành Nội vụ hiệu lực 2026
- **Slug**: 443954
- **Trạng thái**: Chưa có

### 5. Thông tư 04/2026/TT-BDTTG — Hướng dẫn quản lý phát triển vùng dân tộc thiểu số 2026-2030
- **Slug**: 443881
- **Trạng thái**: Chưa có

### 6. Thông tư 10/2026/TT-BTP — Hướng dẫn việc hỗ trợ, tài trợ và quản lý, sử dụng kinh phí hỗ trợ, tài trợ của Quỹ hỗ trợ xây dựng chính sách, pháp luật
- **Slug**: 444428
- **Trạng thái**: Hoàn thiện

---

## VĂN BẢN MỚI PHÁT HIỆN (5) — 2026-08-18 (Lưu trữ)

### 1. Thông tư 115/2026/TT-BQP — Quy định bảo vệ bí mật nhà nước trong Bộ Quốc phòng
- **Slug**: 444329
- **Trạng thái**: Hoàn thiện (Tóm tắt/Trích đoạn)

### 2. Thông tư 64/2026/TT-BXD — Quy định thi đua khen thưởng Bộ Xây dựng
- **Slug**: 444189
- **Trạng thái**: Chưa có

### 3. Thông tư 65/2026/TT-BXD — Định mức kinh tế kỹ thuật khảo sát độ sâu hàng hải
- **Slug**: 444187
- **Trạng thái**: Hoàn thiện

### 4. Thông tư 38/2026/TT-NHNN — Quản lý ngoại hối cho đầu tư nước ngoài tại Việt Nam
- **Slug**: 444098
- **Trạng thái**: Chưa có

### 5. Thông tư 116/2026/TT-BTC — Quy định phí thẩm định tiêu chuẩn hành nghề thừa phát lại
- **Slug**: 443985
- **Trạng thái**: Chưa có

---

## VĂN BẢN MỚI PHÁT HIỆN (1) — 2026-08-16 (Lưu trữ)

### 1. Nghị định 303/2026/NĐ-CP — Sửa đổi Nghị định 32/2024 về quản lý, phát triển cụm công nghiệp

| Trường | Giá trị |
|--------|---------|
| **Số** | 303/2026/NĐ-CP |
| **Loại** | Nghị định |
| **Cơ quan** | Chính phủ |
| **Ban hành** | 01/08/2026 |
| **Hiệu lực** | 15/09/2026 |
| **Chủ đề** | Công nghiệp, Quản lý Nhà nước |
| **LuatVietnam** | `linh-vuc-khac/da-co-bang-so-sanh-nghi-dinh-303-2026-nd-cp-va-nghi-dinh-32-2024-nd-cp-ve-quan-ly-phat-trien-cum-cong-nghiep-883-111417-article.html` |
| **Slug** | 111417 |
| **Nhóm** | Nghị định |
| **Ưu tiên** | Trung bình |
| **Mô tả** | Sửa đổi, bổ sung một số điều của Nghị định 32/2024/NĐ-CP về quản lý, phát triển cụm công nghiệp, quy định 4 loại hình cụm công nghiệp mới. |
| **Trạng thái** | Chưa có |

---

## VĂN BẢN MỚI PHÁT HIỆN (5) — 2026-08-15 (Lưu trữ)

### 1. Nghị định 320/2026/NĐ-CP — Sửa đổi Nghị định 69/2024 về định danh và xác thực điện tử
- **Slug**: 444072
- **Trạng thái**: Chưa có

### 2. Nghị định 318/2026/NĐ-CP — Quy định chi tiết Luật Phục hồi phá sản về hợp đồng phái sinh
- **Slug**: 443951
- **Trạng thái**: Chưa có

### 3. Nghị định 314/2026/NĐ-CP — Quy định hoạt động của sàn dữ liệu
- **Slug**: 443950
- **Trạng thái**: Có (đã có file `van-ban/314-2026-nd-cp.md`)

### 4. Nghị định 317/2026/NĐ-CP — Quy định tổ chức quản lý và sử dụng Quỹ học bổng quốc gia
- **Slug**: 443816
- **Trạng thái**: Chưa có

### 5. Nghị định 311/2026/NĐ-CP — Sửa đổi Nghị định 189/2025 về xử lý vi phạm hành chính
- **Slug**: 443102
- **Trạng thái**: Chưa có

---

## CẬP NHẬT TIẾN ĐỘ CRAWL — 2026-08-16

| Số hiệu | Tên văn bản | Trạng thái | Ngày hoàn thành | Ghi chú |
|---------|-------------|------------|-----------------|----------|
| 35/2026/TT-BCT | Quy định đặc điểm kinh tế - kỹ thuật đối với hàng hóa bình ổn giá, kê khai giá | Hoàn thiện | 2026-08-18 | Pass Quality Gate |
| 315/2026/NĐ-CP | Quy định về kiểm định và kiểm tra an ninh thiết bị kỹ thuật bảo vệ bí mật nhà nước | Hoàn thiện | 2026-08-17 | Pass Quality Gate |
| 312/2026/NĐ-CP | Nghị định về kết hợp an ninh với kinh tế - xã hội... | Hoàn thiện | 2026-08-16 | Pass Quality Gate |
| 136/2026/TT-BCA | Thông tư quy định danh mục sản phẩm khai thác CSDL... | Hoàn thiện | 2026-08-15 | Pass Quality Gate |
| 314/2026/NĐ-CP | Quy định hoạt động của sàn dữ liệu | Hoàn thiện | 2026-08-16 | Pass Quality Gate |
| 10/2026/TT-BTP | Hướng dẫn quản lý, sử dụng kinh phí hỗ trợ Quỹ xây dựng chính sách | Hoàn thiện | 2026-08-20 | Pass Quality Gate |

---
## BÁO CÁO REVIEW QUALITY GATE — 2026-08-17
- **Văn bản review**: 
  - 136/2026/TT-BCA: Pass
  - 312/2026/NĐ-CP: Pass (Đã fix 2 lỗi "ngày l")
  - 314/2026/NĐ-CP: Pass
- **Kết luận**: Các văn bản đã review đạt chuẩn OCR Quality Gate.

---
## OCR QUALITY GATE REVIEW — 2026-08-20 (Đệ #3 Subagent)

### File PASS hoàn toàn (OCR issues = 0, Articles OK, Chapters OK)
| File | Lines | Articles | Chapters | Đánh giá |
|------|-------|----------|----------|----------|
| `van-ban/10-2026-tt-btp.md` | 648 | 27 (1-27) | 5 (I-V) | **PASS** |

> **Lưu ý**: 3 phát hiện `ngày l` là false positive (thực tế là `ngày làm` - ngày làm việc, thuật ngữ pháp lý chuẩn).

---
## FILE CẦN REFACTOR (Đệ #4 Reviewer - 2026-08-18)

| # | File | Kích thước | Last Modified | Vấn đề | Trạng thái |
|---|------|-----------|---------------|--------|------------|
| 1 | `van-ban/tai-chinh/thong-tu-105-2026-tt-btc.md` | 6,020 bytes | 2026-08-04 | Metadata có "Đang cập nhật" cho Ngày công báo & Số công báo | Chờ xử lý |
| 2 | `van-ban/2026-archive/86-2026-TT-BTC-quan-ly-thue-hang-hoa-xnk.md` | 1,574 bytes | 2026-07-23 | Stub file - status "Đang cập nhật (stub)", chờ OCR từ PDF | Chờ xử lý |
| 3 | `van-ban/xay-dung/62-2026-tt-bxd-qcvn-32-duong-sat-do-thi-metro.md` | 1,557 bytes | 2026-08-04 | Stub file - "Đang cập nhật nội dung", QCVN 32:2026/BXD chưa có toàn văn | Chờ xử lý |

> Tiêu chí: metadata "Đang cập nhật" + < 10KB + lastedit > 7 ngày

---
## OCR QUALITY GATE REVIEW CHI TIẾT (Đệ #4 Reviewer - 2026-08-18)

### File PASS hoàn toàn (OCR issues = 0, Articles OK, Chapters OK)
| File | Lines | Articles | Chapters | Đánh giá |
|------|-------|----------|----------|----------|
| `van-ban/hanh-chinh/nghi-dinh-312-2026-nd-cp.md` | 155 | 17 (1-17) | 4 | **PASS** |
| `van-ban/314-2026-nd-cp.md` | 162 | 8 (1-8) | 2 | **PASS** |
| `van-ban/tai-chinh/nghi-dinh-316-2026-nd-cp.md` | 139 | 1 | 0 | **PASS** |
| `van-ban/tai-chinh/nghi-dinh-318-2026-nd-cp.md` | 95 | 10 (1-10) | 0 | **PASS** |

### File CẦN SỬA OCR (lỗi "ngày l" -> "ngày 1")
| File | Lines | Articles | OCR Issues | Chi tiết lỗi |
|------|-------|----------|------------|--------------|
| `van-ban/315-2026-nd-cp.md` | 200 | 17 (1-17) | **5** | `ngày l` (5 lần, dòng 113, 116, 125, 143, 153) |
| `van-ban/cong-an/127-2026-tt-bca-qcvn-13-co-so-giam-giu.md` | 716 | 4 (1-4) | **1** | `ngày l` (dòng 434) |
| `van-ban/ngoai-giao/nghi-dinh-293-2026-nd-cp-apostille-hieu-luc-ngay.md` | 590 | 37 (1-37) | **13** | `ngày l` (13 lần, dòng 201, 235, 269, 271, 273...) |
| `van-ban/ngan-hang/thong-tu-04-2026-tt-nhnn-bao-hiem-tien-gui.md` | 241 | 15 (1-15) | **10** | `ngày l` (10 lần, dòng 67, 108, 116, 118, 126...) |

### File CẦN CHUẨN HÓA CẤU TRÚC
| File | Vấn đề | Chi tiết |
|------|--------|----------|
| `van-ban/hanh-chinh/thong-tu-136-2026-tt-bca.md` | 5 suspicious articles | Dùng `**Điều X.**` thay vì `### Điều X.` (dòng 36, 39, 43, 85, 89) |
| `van-ban/lao-dong/nghi-dinh-162-2026-nd-cp-dieu-chinh-luong-huu-tro-cap-bhxh-tro-cap-hang-thang.md` | 6 suspicious articles + 1 OCR | Dùng `**Điều X.**` (dòng 95, 119, 131, 139, 156) + ký tự `ø` (dòng 214) |

> **Lưu ý quan trọng:** File `van-ban/315-2026-nd-cp.md` được tracking ghi là "Pass Quality Gate" nhưng thực tế còn 5 lỗi `ngày l`. Cần fix trước khi merge.

> **Tổng lỗi "ngày l" phát hiện: 29 lỗi**
> **Tổng lỗi ký tự rác khác: 1 (ø)**