...
👉 [Danh sách 45 Chủ đề](phap-dien/markdown/00-danh-sach-chu-de.md)
...
- [{{ demuc }}](phap-dien/markdown/) - *Đang cập nhật*
{% endfor %}

👉 [Xem tất cả 306 Đề mục](phap-dien/markdown/README.md)
...
| **SQLite (Simple)** | Database cũ | `phap-dien/sqlite/phapdien_simple.db` | 18,649 |
| **Markdown** | Hiển thị web | `phap-dien/markdown/*.md` | - |
| **JSON** | API, mobile apps | `phap-dien/database/json/*.json` | - |
| **Search Index** | Tìm kiếm keywords | `phap-dien/database/search/keywords.json` | - |
...
👉 [Xem đầy đủ với số đề mục](phap-dien/markdown/00-danh-sach-chu-de.md)
...
2. **Copy** file `jsonData.js` vào thư mục `phap-dien/json/`
3. **Chạy build script**:
   ```bash
   cd phap-dien/scripts/
