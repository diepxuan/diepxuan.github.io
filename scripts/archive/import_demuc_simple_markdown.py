#!/usr/bin/env python3
"""
Import nội dung đề mục từ thư mục demuc và convert HTML sang Markdown đơn giản
Không dùng JSON column, chỉ dùng TEXT
"""
...
def html_to_markdown_simple(html_content):
    ...
    if markdown:
        ...
        matches = re.findall(provision_pattern, markdown)
...
def import_demuc_markdown(connection, demuc_mapping):
    ...
    # Tạo bảng mới nếu chưa có (đơn giản hơn)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS de_muc_markdown_simple (
    ...
    for file_idx, file_name in enumerate(files, 1):
        ...
        if uuid in demuc_mapping:
            ...
            try:
                ...
                # Insert vào database
                cursor.execute("""
                    INSERT INTO de_muc_markdown_simple 
...
def main():
    print("=== IMPORT NỘI DUNG ĐỀ MỤC (SIMPLE MARKDOWN) ===")
    ...
    try:
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT COUNT(*) FROM de_muc_markdown_simple")
        total = cursor.fetchone()[0]
        print(f"Tổng records trong de_muc_markdown_simple: {total}")
        ...
        cursor.execute("""
            ...
            FROM de_muc_markdown_simple dmm 
        ...
        print(f"de_muc_markdown_simple (Markdown): {total} records")
